...
export default function LightSetting({ isFix, current, onChange }) {
  ...
  const onSave = () => {
    let repeatMode = isFix ? "fix" : "repeat";
    useUpdateSetting(repeatMode, "production", "none").then(res => {
      if (res?.data?.isSuccess) {
        if (isFix) {
          useUpdateFix(l1, l2, l3, l4, l5, l6, l7, l8).then(r => {
            if (r?.data?.isSuccess) {
              Taro.atMessage({
                message: "保存成功",
                type: "success"
